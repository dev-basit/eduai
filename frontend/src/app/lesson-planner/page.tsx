            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <span
                  ...
                  style={{ background: subjectColor(p.subject) }}
                />
              ...
              {visibleTopics.length > 0 && (
                <div className="flex flex-wrap gap-1 ml-4 mb-1">
                  {visibleTopics.map((t) => (
                    <span
                      key={t}
                      className="text-xs px-1.5 py-0.5 rounded-md truncate max-w-[80px]"
                      style={{
...
function PlanForm({ onQuizReady }: { onQuizReady: (form: FormState, quiz: PlannerQuiz) => void }) {
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                type="button"
                onClick={() => setForm({ ...form, days: Math.min(60, form.days + 1) })}
                className="w-7 h-7 rounded-lg text-sm font-bold flex items-center justify-center"
                style={{ background: '#f3f4f6', color: '#374151' }}
              >
                +
    ...
    <div className="max-w-2xl mx-auto py-8 px-6">
      ...
      >
        {loading ? (
          <span className="flex items-center justify-center gap-2">
            <span className="w-4 h-4 rounded-full border-2 border-white border-t-transparent animate-spin" />
            Analysing your answers, building plan & resources
          </span>
        ) : answered === 0 ? (
          'Answer at least one question to continue'
        ) : (
          'Submit & Generate Plan & Resources →'
        )}
      </button>
    </div>
  );
                ...
                <ul className="space-y-1.5">
                  {items?.map((item, j) => (
                    <li key={j} className="text-xs text-gray-600 flex items-start gap-2 leading-relaxed">
                      <span className="shrink-0 mt-0.5 text-gray-300">{icon}</span>
                      {item}
                    </li>
                  ))}
                </ul>
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 py-8 max-w-3xl mx-auto w-full">
      ...
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          <span
            className="text-xs font-bold px-3 py-1 rounded-full text-white inline-block mb-2"
      ...
      {improveError && (
        <div
          className="mb-4 text-xs px-4 py-3 rounded-xl"
      ...
      {/* Week overview */}
      <div
        className="rounded-2xl px-5 py-4 mb-5 text-sm leading-relaxed"
  ...
  return (
    <div className="flex h-full min-h-screen" style={{ background: '#f1f2f6' }}>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        {mode === 'view' && selected && (
          <PlanDetail plan={selected} onNew={handleNew} onImprove={handleImprove} />
        )}
      </main>
    </div>
  );
