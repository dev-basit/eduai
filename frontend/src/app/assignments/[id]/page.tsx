...
function AnsweringPhase({ assignment, onSubmit, loading, error }: {
    ...
    <div className="max-w-2xl mx-auto p-8">
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {[c.subject, c.topic, assignment.difficulty, `${c.total_marks} marks`].map((tag) => (
            <span key={tag} className="text-xs font-medium px-2.5 py-1 rounded-full capitalize"
              style={{ background: "#f3f4f6", color: "#6b7280" }}>{tag}</span>
          ))}
        </div>
      </div>

      <div className="text-sm rounded-xl px-4 py-3 mb-6 leading-relaxed"
      ...
      <div className="mb-5">
        <div className="flex justify-between text-xs text-gray-400 mb-1">
          <span>{answered} of {c.questions?.length ?? 0} answered</span>
        </div>
      ...
      <div className="space-y-4 mb-6">
        {c.questions?.map((q, i) => (
          <div key={q.id} className="bg-white rounded-2xl p-5"
            style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.07)", border: `1px solid ${answers[String(q.id)] ? "#c4b5fd" : "#f3f4f6"}` }}
          >
...
function ReviewingPhase({ submission, onCounterSubmit, onSkip, loading, error }: {
  ...
  const wrongResults = fb.question_results.filter((qr) => !qr.is_correct);
  ...
  return (
    <div className="max-w-2xl mx-auto p-8">
      ...
      >
        <ScoreRing pct={fb.percentage} />
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-3xl font-black"
              style={{ color: fb.percentage >= 80 ? "#16a34a" : fb.percentage >= 60 ? "#d97706" : "#dc2626" }}
            >{fb.grade}</span>
            <span className="text-sm font-medium px-3 py-1 rounded-full"
              style={{
                background: fb.percentage >= 80 ? "#f0fdf4" : fb.percentage >= 60 ? "#fffbeb" : "#fef2f2",
                color: fb.percentage >= 80 ? "#166534" : fb.percentage >= 60 ? "#92400e" : "#991b1b",
              }}
            >{fb.total_score} / {fb.max_score} marks</span>
          </div>
      ...
      <div className="space-y-4 mb-6">
        {fb.question_results.map((qr: QuestionResult) => (
          <div key={qr.question_id} className="bg-white rounded-2xl p-5"
            style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06)", border: `1px solid ${qr.is_correct ? "#bbf7d0" : "#fecaca"}` }}
          >
            <div className="flex items-start gap-3">
              <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0 mt-0.5"
                style={{ background: qr.is_correct ? "#16a34a" : "#dc2626" }}
              >{qr.is_correct ? "✓" : "✗"}</span>
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <p className="text-sm font-medium text-gray-800">{qr.question}</p>
                  <span className="text-sm font-bold shrink-0"
                    style={{ color: qr.is_correct ? "#16a34a" : "#dc2626" }}
                  >{qr.score}/{qr.max_score}</span>
                </div>
                ...
                {qr.is_correct && qr.counter_question && (
                  ...
                  >
                    ...
                    <textarea
                      ...
                      onChange={(e) => setCounterAnswers({ ...counterAnswers, [String(qr.question_id)]: e.target.value })}
                    />
                  </div>
                )}
...
function CompletePhase({ submission, counterCheck }: {
  submission: AssignmentSubmission;
  ...
  return (
    <div className="max-w-2xl mx-auto p-8">
      <Link href="/assignments" className="text-xs text-gray-400 hover:text-gray-600 transition-colors block mb-6">
        ← New Assignment
      </Link>
      ...
      >
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            >{fb.total_score} / {fb.max_score}</span>
          </div>
          <p className="text-sm text-gray-500">{fb.overall_feedback}</p>
        </div>
      </div>
      ...
      {counterCheck && counterCheck.results.length > 0 && (
        <div className="mb-6">
          <div className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Understanding Check</div>
          <div className="space-y-3">
            {counterCheck.results.map((cr) => (
              <div key={cr.question_id} className="bg-white rounded-xl p-4"
                style={{
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium" style={{ color: cr.understood ? "#166534" : "#991b1b" }}>
          ...
          {counterCheck.overall_understanding && (
            <div className="mt-3 px-4 py-3 rounded-xl text-xs leading-relaxed"
              style={{ background: "#ede9fe", color: "#5b21b6" }}
            >
              <span className="font-semibold">AI Assessment: </span>{counterCheck.overall_understanding}
            </div>
          )}
        </div>
      )}
          ...
          <ul className="space-y-1.5">
            {fb.strengths?.map((s: string, i: number) => (
              <li key={i} className="flex items-start gap-2 text-sm text-gray-700">
                <span className="text-emerald-500 font-bold mt-0.5 shrink-0">✓</span>{s}
              </li>
            ))}
          </ul>
        </div>
        <div className="bg-white rounded-2xl p-5" style={{ boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
          ...
          <ul className="space-y-1.5">
            {fb.areas_for_improvement?.map((a: string, i: number) => (
              <li key={i} className="flex items-start gap-2 text-sm text-gray-700">
                <span className="text-amber-500 font-bold mt-0.5 shrink-0">→</span>{a}
              </li>
            ))}
          </ul>
        </div>
...
export default function AssignmentPage() {
  ...
  useEffect(() => {
    assignmentService.getById(id).then(async (a) => {
      setAssignment(a);
      try {
